...
bool ellenoriz(datum d) { // datum tartalmi ellenorzese
...
int evNapja(datum d) { // ev napjanak meghatarozasa
...
string hetNapja(datum d) { // het napjanak szamitasa
  int hn;
...
int bazis(datum d, int bazisEv) { // bazisEv.01.01 ota eltelt napok szama
  int b = 0;
...
int kulonbseg(datum tol, datum ig) {
  int bazisEv = tol.ev < ig.ev ? tol.ev : ig.ev;
  return bazis(ig, bazisEv)-bazis(tol, bazisEv);
}
