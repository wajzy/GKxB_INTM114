...
int main() {
  teglalap tt[MAXALAK];
  int db=0, k; bool folytat=true;
  cout << "Rajzprogram - adja meg a téglalapok adatait!\n";
  while(db<MAXALAK and folytat) {
    do {
      cout << db+1 << ". teglalap BF sarok X: [" << MINX 
    ...
    if(folytat) {
      ...
      tt[db].ja.x = beker(db+1, "JA sarok X", tt[db].bf.x+1, MAXX);
      tt[db].ja.y = beker(db+1, "JA sarok Y", tt[db].bf.y+1, MAXY);
